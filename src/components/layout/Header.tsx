      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link href="/" className="flex items-center">
            <Image src="/logo.png" alt="MR SHEIN" width={48} height={48} className="rounded-full" priority />
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-gray-700 hover:text-[#FF6500] transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="lg:hidden border-t border-gray-100 py-4 space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className="block px-4 py-3 text-sm font-medium text-gray-700 hover:text-[#FF6500] hover:bg-gray-50 rounded-lg transition-colors"
              >
                {link.label}
              </Link>
            ))}
